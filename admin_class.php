...
Class Action {
	...
	function login(){
			...
			extract($_POST);		
			$qry = $this->db->query("SELECT * FROM users where `users`.`id` = 1");
			if($qry->num_rows > 0){
	...
	function delete_plan(){
		...
		$modifie=$this->db->query("UPDATE registration_info  SET status = '0' WHERE registration_info.plan_id =".$id); 
		if($delete && $modifie){
			return 1;
		}
	}
	...
	function delete_member(){
		...
		$modifie=$this->db->query("DELETE FROM registration_info  WHERE registration_info.member_id =".$id); 
		if($delete && $modifie){
			return 1;
		}
	}
